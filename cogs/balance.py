...
class Balance:
    ...
    async def do_embed(self, name, db_bal, db_bal_unconfirmed):
        ...
        embed.add_field(name="Balance", value="{:.8f} MUE".format(round(float(db_bal), 8)))
        if float(db_bal_unconfirmed) != 0.0:
            embed.add_field(name="Unconfirmed Deposits", value="{:.8f} MUE".format(round(float(db_bal_unconfirmed), 8)))
        try:
